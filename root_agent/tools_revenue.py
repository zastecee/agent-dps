...
with open(
  "FINAL_V_DRA_DAILY_REVENUE_SELECT_VW.csv",
  mode="r",
  encoding="iso-8859-1",
  newline="",
) as file:
...
def get_total_custumers_per_day(kpi: str, date: str) -> dict:
  try:
    total = 0
    for item in records:
      if item["SERVICE_NAME"] == kpi and item["OC_DATE"] == date:
        total = float(item["DR_PREPAID"]) + total
...
# print(get_total_custumers_per_day('Device Finance - Repayment', "06-AUG-26"))
# print(get_total_custumers_per_day('Device Financing', "06-AUG-26")) ----
# CBU - Postpaid -----
# print(get_total_custumers_per_day('Interconnect', "06-AUG-26"))
# print(get_total_custumers_per_day('Core', "06-AUG-26"))
# print(get_total_custumers_per_day('Financial Services', "06-AUG-26"))
# print(get_total_custumers_per_day('Others - M-Pesa', "06-AUG-26"))
# MPesa ------
# print(get_total_custumers_per_day('Mpesa', "06-AUG-26")) ---- deve somar com Others - M-Pesa
# VB -----------
...
def get_kpi_month_to_date_total(kpi_name: str, target_date: str) -> dict:
  try:
    ...
    for record in records:
      if record["SERVICE_NAME"] != kpi_name:
        continue

      record_date = datetime.strptime(record["OC_DATE"], "%d-%b-%y").date()

      if month_start_date <= record_date <= target_date_obj:
        month_to_date_total += float(record["DR_TOTAL"])
    return {"date": target_date, "kpi": kpi_name, "total": month_to_date_total}
  except Exception as e:
...
def get_customers_month_on_month_metrics(kpi_name: str, target_date: str) -> dict:
  try:
    ...
    previous_month_total = get_kpi_month_to_date_total(kpi_name, previous_month_date)[
      "total"
    ]
    ...
    return {"error": f"calculate_kpis failed: {e}"}
...
def get_kpi_year_on_year_metrics(kpi_name: str, target_date: str) -> dict:
  try:
    ...
    previous_year_total = get_kpi_month_to_date_total(kpi_name, previous_year_date)[
      "total"
    ]
    ...
    return {"error": f"get_kpi_year_on_year_metrics failed: {e}"}
